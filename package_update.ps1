...
function list_package_system {
    ...
    $file_list_package = "$HOME\winget_$date"
    ...
    $list_packages_to_update = $list_packages_object | Where-Object { $null -ne $_.Available } | Select-Object -ExpandProperty Name
...
function install_package_2 {
    ...
    Write-Host @"
###################################################################
# WINGET - Installation des paquets 2 #############################
###################################################################
